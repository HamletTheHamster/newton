...
There are no tests or linters configured. Deploy happens automatically on `git push` via Netlify CI.

For local dev with Netlify Functions (the Claude API proxy), use `netlify dev` instead of `npm run dev` — this runs the function at `/.netlify/functions/claude` alongside the Vite dev server.

## Environment

One required env var: `VITE_FIREBASE_API_KEY`. In local dev, put it in `.env` (gitignored). In Netlify, it's set in the dashboard. The Anthropic API key (`ANTHROPIC_API_KEY`) lives only in the Netlify environment (used server-side by the function).
...
Two calls, both proxied through `netlify/functions/claude.js` (which just forwards to `api.anthropic.com/v1/messages` with the server-side API key):
...
All styles are inline. A shared `s` object at the top of `newton.jsx` holds reusable style objects (`s.card`, `s.input`, `s.btnPri`, etc.). Theme constants: `BG`, `CARD`, `TEAL`, `MUTED`, `BORDER`.
