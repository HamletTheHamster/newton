...
src/
  ...
  screens/instructor/
    Modules.jsx                  — instructor Home tab: module authoring (module ⋮ menu: inline
                                   ...
                                   rename, replace file, delete; full-div drag/drop with correct
                                   gap targeting; animated drop gaps own their onDragOver so jitter
                                   is eliminated; release dates; file uploads)
...
classes/{classId}/
  ...
  announcementReads/        — { [studentId]: { [annId]: "<ISO timestamp>" } }  — read receipts (phase 9.5); legacy `true` values from the 6.9-era version still parse
...
| **4** | ✅ done | **Announcements**: instructor compose UI; broadcast list page. The unread popup modal and sidebar badge (per-student read tracking via `announcementReads/`) were added here and removed in Phase 6.9 (buggy on class switch, no instructor-facing payoff); the popup came back in Phase 9.5, this time with receipts the instructor can actually read. |
...
| **9** | ✅ done | **Email broadcast**: `parseRoster` captures Preferred Email column; manual add form gains email field; roster table shows editable email column; student settings gains email self-service field; instructor can edit any student's email inline; new `netlify/functions/send-email.js` proxies to Resend API; `AnnouncementEditor` gains email checkbox (default on, shows address count); subject auto-prefixed with `term courseNumber:` from syllabus fields. Domain `notifications.newtonphy.com` verified with Resend; app live at `newtonphy.com` via Netlify DNS. |
| **9.5** | ✅ done | **Announcement read receipts + unread popup**: `classes/{classId}/announcementReads/{studentId}/{annId}` now stores an ISO timestamp. A student signing in to the portal sees any announcement they have not seen yet in a modal on top of the course (`NewAnnouncementsModal`); dismissing it, or opening the Announcements page, records the view. The instructor's Announcements tab reads the whole node lazily and shows "Viewed by N of M" per announcement, expanding to name both who has seen it and who has not. Pure helpers in `src/announcements.js`. The 6.9 class-switch bug is designed out: the loaded receipts carry the `{classId, studentId}` they were fetched for and the popup shows nothing until that pair matches the current class. `READ_TRACKING_SINCE` keeps announcements posted before the feature from popping at everyone at once, and from being reported as unviewed when they simply were not tracked. |

## Notes for future sessions

- **Gradebook architecture**: `buildGradebookAssignments(mergedModules, quizzes, assignmentCategories, manualAssignments, assignmentNameOverrides, assignmentOrderOverrides)` in `utils.js` returns a flat sorted assignment list. Module items get `order = modIdx*100 + itemIdx`; manual assignments use their stored `order` field (defaults 9999 if absent). `assignmentOrderOverrides` (sixth param, `{[id]: number}`) overrides any natural order — set when the instructor drag/drops columns in `Gradebook.jsx` and persisted to `assignmentOrderOverrides/` in RTDB. The combined list is sorted by `order` so exams and labs interleave correctly with quiz/HW columns. Default assignments (Midterm, Final, and two labs per lecture week — 26 for PHY 215, 28 elsewhere; see `src/lab-schedule.js`) are seeded into `manualAssignments/` on first class load when the key is empty; the lab count is the one part that varies by course type. `calcGrades({assignments, categories, scores, excused})` computes weighted totals. The gradebook pre-filters to "active" assignments (submitted OR past dueDate) before passing to `calcGrades` to avoid inflating the denominator. Grade data flows: `gradeOverrides/{studentId}/{assignmentId}` → `{score?, excused?}`; `gradeCategories/{catId}` → `{id, name, weight, order}`; `manualAssignments/{id}`, `assignmentNameOverrides/{id}`, and `assignmentOrderOverrides/{id}` are all class-level. Both `Gradebook.jsx` and `StudentGrades.jsx` receive `assignmentNameOverrides` from `App.jsx` so names stay consistent.
- **Module file pickers (`Modules.jsx`)**: two always-mounted hidden `<input type="file">` elements live at the component root (`addFileInputRef`, `replaceInputRef`) so a menu click can call `.click()` **synchronously inside the user gesture** — the OS dialog must not be opened from an effect after a state-driven render. (1) Choosing **File** in a module's ⋮ add menu opens the picker immediately; the add bar still renders behind it with the file staged, a title field, and the Upload button (its own "Choose file…" label remains as the fallback if the dialog is cancelled). (2) A `file` item's ⋮ menu has **⇄ Replace file…**, which uploads a new file **in place**: it keeps the same `uploadId` (so the module item, its position, and any student-facing link are untouched), rewrites the `uploads/{uploadId}` metadata (`createdAt` preserved, `updatedAt` added), refreshes the item title only when it was still the old filename, and best-effort deletes the superseded Storage object — **skipped when the new path equals the old one** (same sanitized filename ⇒ the upload already overwrote it, so deleting would destroy the new file). Progress renders in the row's subtitle; failures render as red text under it. Replacing reuses `onUploadFile`/`onSaveUpload` plus a new `onDeleteStorage` prop (`fbDeleteStorage`) — deliberately *not* `onDeleteUpload`, which would drop the RTDB node the replacement just wrote.
...
- **Student email**: `parseRoster` now captures the "Preferred Email" column from MyMercer CSVs. The `email` field is optional on roster entries. Instructors can also edit emails inline in the roster table or set them on manual-add; students can update their own email in Settings.
