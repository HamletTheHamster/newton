...
**Not yet implemented / remaining work** (see [homework-roadmap.md](homework-roadmap.md)): notably, **retakes are not yet practice-gated** — reopening a completed homework currently saves another graded submission; it should instead run practice-only (no re-grade) like quizzes. Also pending: real `hw2…hwN` content, an instructor grading-settings UI, per-part gradebook overrides, the `inst-sub-detail` homework breakdown, and image-answer problems.
