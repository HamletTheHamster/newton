...
async function evaluateAnswer(question,answer,history,imageData){
  ...
  const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system,messages:[...history,{role:"user",content:userContent}]})});
  const data=await res.json();
  const text=data.content?.map(b=>b.text||"").join("")||"";
...
export default function App(){
  ...
  const toggleChecked=async subId=>{
    ...
    else{nc[subId]=true;await saveSubs(submissions.map(s=>s.id===subId?{...s,dialogue:null}:s));}
